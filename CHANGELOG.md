...
All notable changes to DraftGo Launcher will be documented in this file.
...
Initial independent release as **DraftGo Launcher**. The project is now
published under its own package name, process name, app identifier, deep-link
scheme, icons, configuration directory, and release assets, and no longer
carries the `cc-switch` / `CC Switch` branding.
...
- **Grok Build Joins the Failover Tabs and the Environment-Conflict Check**: The failover settings gain a fourth tab for Grok Build alongside Claude Code, Codex and Gemini, and the startup environment-conflict banner now detects a shell-exported `XAI_API_KEY` or `GROK_DEFAULT_MODEL` — variables that silently override whatever provider is selected. Detection distinguishes exact names from prefixes, so DraftGo Launcher's own `GROK_BIN_DIR` and `GROK_HOME` are not reported.
...
- The mirrored DeepSeek catalog declares a minimum Codex client version of 0.144.0, which DraftGo Launcher does not verify — the freeform `apply_patch` registration it carries needs that release or newer. The generated catalog file also grows to roughly 75 KB for the two mirrored models, since each carries the full harness text.
...
- **Release Assets Mirrored to Cloudflare R2 for github.com/draftgo/DraftGo-Launcher Downloads**: Release automation mirrors every installer to an R2 bucket behind `dl.github.com/draftgo/DraftGo-Launcher` and generates the manifest the github.com/draftgo/DraftGo-Launcher download page consumes — per-file SHA-256 checksums, sizes, and the release's real publish date — with immutable caching for versioned assets, a five-minute TTL on the root manifest, and pruning beyond the five most recent versions. Forks without the R2 credentials skip the sync entirely.
...
- New built-in pricing rows (`claude-opus-5`, `gemini-3.6-flash`, `grok-4.5-build`) are appended insert-if-absent on next launch — this seeding never overwrites a price you edited. A `~/.draftgo-launcher/model-pricing.json` file is created (empty) to hold manual overrides and deletions made from this version onward — earlier price edits are not migrated into it, so re-save any you want protected against a database rebuild. Automatic models.dev sync stays off until you enable it; once enabled, it is the one path that does overwrite matching prices, built-in and hand-edited alike.
...
- The updater endpoint list ships inside the app binary, so existing installations keep checking GitHub until they have updated once to a build containing this change; from then on the `dl.github.com/draftgo/DraftGo-Launcher` mirror is preferred with GitHub as fallback.
...
- **Volcengine/Doubao/BytePlus Website Links Restored to Invite Pages**: Reverts the v3.16.5 change (`56248087`) that had pointed the `火山Agentplan`, `DouBaoSeed`, and `BytePlus` presets' `websiteUrl` at their product homepages. Per the commit, the `websiteUrl` for these three presets is intentionally the same draftgo-attributed campaign/invite link as `apiKeyUrl`, so it is restored across all six app preset files — `火山Agentplan` back to its `activity/codingplan` link, `BytePlus` to its `product/modelark` link, and `DouBaoSeed` to the Ark console API-key page, each carrying the `utm_*` attribution params. The `apiKeyUrl` links were already left intact by the reverted commit and are unchanged.
- **Code0.ai Invite Link Updated to Agent Register URL**: The Code0.ai sponsor's `apiKeyUrl` moves from the `?source=draftgo` referral to the new agent-register invite link `https://code0.ai/agent/register/B2XHxGjGmRvqgznY`, updated across all seven app presets and the Code0 sponsor rows in all four READMEs (`README.md`, `README_ZH.md`, `README_JA.md`, `README_DE.md`). The bare `code0.ai` API endpoint stays untouched.
...
- **Harden Release Supply Chain**: Added a `.github/CODEOWNERS` that, together with branch protection's Code Owners rule, requires owner review before any PR merges to `main` (with `/.github/` and `/src-tauri/` pinned explicitly alongside the global `*` fallback, all to `@draftgo`), gated the release job behind a `release` environment in `release.yml` so signing secrets unlock only after manual approval, and removed `.github` from `.gitignore` so the previously untracked `labeler.yml` and `workflows/labeler.yml` are now versioned.
...
- **Documented the `DRAFTGO_LAUNCHER_GDK_BACKEND` Escape Hatch in README and User Manual**: Added an FAQ entry for the opt-in `DRAFTGO_LAUNCHER_GDK_BACKEND` environment variable (see the corresponding Fixed entry) across all four README locales (`README.md`, `README_ZH.md`, `README_JA.md`, `README_DE.md`) and the zh/en/ja user-manual troubleshooting pages (`docs/user-manual/{zh,en,ja}/5-faq/5.2-questions.md`), explaining how Wayland + NVIDIA users can switch back to native Wayland when the webview goes click-dead and black-screens on resize, and how tiling-Wayland users can set it to `x11` for the inverse input bug.
- **Overseas Kimi READMEs Point to platform.kimi.ai With a Coding Plan Link**: Updated the Kimi K2.7 Code partner section across the English, German, and Japanese READMEs so the banner image and both inline CTAs point to `https://platform.kimi.ai?aff=draftgo-launcher` instead of the old `platform.kimi.com` host, keeping the `aff=draftgo-launcher` referral tag intact. All four localized READMEs (`README.md`, `README_DE.md`, `README_JA.md`, `README_ZH.md`) also gained a new line promoting the Kimi For Coding subscription plan linked to `https://www.kimi.com/code/?aff=draftgo-launcher`; note the ZH README kept its existing `platform.kimi.com` link and only received the Coding Plan addition.
- **GitHub Global Top-100 Milestone Banner in v3.16.4 Release Notes**: Prepended a celebratory callout blockquote to all three localized v3.16.4 release notes (`docs/release-notes/v3.16.4-en.md`, `-ja.md`, `-zh.md`) announcing that DraftGo Launcher has entered the global top 100 GitHub projects by star count, thanking users, contributors, and stargazers. This is a docs-only addition to the release-notes header and does not change any product behavior.
...
- **Simplified Homebrew Install Instructions**: The installation guide no longer instructs users to run `brew tap draftgo/draftgo` before `brew install --cask draftgo-launcher`; the deprecated tap step was removed from the en/ja/zh user manuals so the cask installs directly. (#4319)
...
- **Codex CLI Models Endpoint**: The local proxy now answers `GET /v1/models`, which Codex CLI probes at startup, returning the draftgo-launcher-managed Codex model catalog. A stale-catalog guard parses the live `config.toml` and only serves the catalog when `model_catalog_json` still references the draftgo-launcher-owned file, so a leftover catalog from a previous provider is not advertised (#3818).
...
- **Codex Chat Truncated Stream Detection**: When a Chat Completions upstream ends a stream without a `finish_reason` or `[DONE]`, DraftGo Launcher no longer reports it as a normal completion — it finalizes normally only when the stream truly finished, emits an incomplete (`max_output_tokens`) response when partial output was produced, and emits a failed `stream_truncated` event when nothing was produced. Late-arriving reasoning is also attached to still-active streamed tool calls.
...
- **Codex Model Catalog Path Portability**: Codex now writes only the relative filename `draftgo-launcher-model-catalog.json` to `config.toml` instead of an absolute path (Codex CLI resolves it from the config directory), fixing the model catalog breaking on WSL and symlinked setups where the absolute path could not be translated (#3614).
...
- **Windows Tray Icon Residue on Exit**: Quitting DraftGo Launcher on Windows could leave a dead tray icon until hovered; the app now removes the tray icon before exiting so it disappears cleanly (#3797).
- **Windows Taskbar Icon**: Set an explicit Windows AppUserModelID at runtime and stamped the installer's desktop and start-menu shortcuts with the same ID and product icon, so DraftGo Launcher shows the correct icon and groups properly in the taskbar (#3457).
...
- **Codex Proxy Error Diagnostics**: Codex forwarding failures now return richer JSON errors with provider, model, endpoint, upstream status, stable `draftgo_launcher_*` codes, and HTTP statuses aligned with the canonical `ProxyError` response mapping.
...
- **Codex Model Mapping Table**: Codex provider forms now expose a model catalog (model + display name + context window per row) that is the single source of truth for the upstream model list, projected to `~/.codex/draftgo-launcher-model-catalog.json`.
...
- **Homebrew Official Cask Installation**: Installation simplified to `brew install --cask draftgo-launcher` now that DraftGo Launcher is in the official Homebrew repository; the personal-tap requirement was removed from all READMEs.
...
- **Claude Desktop Third-Party Provider Switching via Proxy Gateway**: Added a dedicated Claude Desktop surface that brokers third-party Claude providers through DraftGo Launcher's in-app proxy, with a routing-support badge for providers that need it, role-based model route mapping locked to `sonnet` / `opus` / `haiku`, Copilot/Codex OAuth provider reuse, a redesigned Claude Code import flow, app-switcher differentiation between "Claude Code" and "Claude Desktop", and 44 provider presets translated from the Claude Code catalog.
...
- **Claude Desktop Operational Notes**: Switching a Claude Desktop provider now writes DraftGo Launcher's managed 3P profile and requires restarting Claude Desktop to take effect; proxy-mode providers require DraftGo Launcher Local Routing to stay running.
...
- **DraftGo Launcher Brand Surface Unified to github.com/draftgo/DraftGo-Launcher**: All in-app and README references now point at github.com/draftgo/DraftGo-Launcher as the sole official website; the release notes template also surfaces github.com/draftgo/DraftGo-Launcher.
...
- **Release Notes Template**: Surfaces `github.com/draftgo/DraftGo-Launcher` in the release notes template.
- **Brand Surface**: Documented `github.com/draftgo/DraftGo-Launcher` as the sole official website across READMEs and in-app references.
...
- **Hermes Config Health Scanner**: Removed the in-app Hermes config health scanner, warning banner, `scan_hermes_config_health` command, `HermesHealthWarning` type, and `HermesWriteOutcome.warnings` payload. DraftGo Launcher now keeps the Hermes surface focused on active provider display, provider switching defaults, memory editing, and launching the Hermes Web UI for deep configuration.
...
- **Codex OAuth Responses SSE Aggregation**: Non-streaming Anthropic clients now receive JSON even when the ChatGPT Codex upstream forces OpenAI Responses SSE; DraftGo Launcher aggregates the upstream SSE events before running the non-streaming transform (#2235).
...
- **Hermes Memory Panel**: Added a Memory panel for editing `MEMORY.md` and `USER.md` directly from DraftGo Launcher, with an enable switch, character-count limits, and a live save flow. Replaces the Prompts entry for Hermes.
...
- **Application-Level Window Controls**: Added an opt-in setting to render DraftGo Launcher's own minimize / toggle-maximize / close buttons instead of the system decorations, materially improving the experience on Linux Wayland where compositor-drawn buttons can become inert.
...
- **OpenClaw Config Directory Override**: Added a settings option to point DraftGo Launcher at a custom `openclaw.json` location.
- **Hermes Config Directory Override**: Added a settings option to point DraftGo Launcher at a custom `~/.hermes/config.yaml` location, backed by data-driven dispatch.
...
- **Hermes Deep Config Delegation**: Deep YAML knobs are now delegated to the Hermes Web UI via a direct launch action, rather than duplicated in the DraftGo Launcher form.
...
- **Tray Menu ID Constant**: The tray identifier moved from the hardcoded string `"main"` to a `TRAY_ID` constant (`"draftgo-launcher"`) across all call sites.
...
- **Hermes YAML Pollution & OAuth MCP Auth Drop**: Round-tripping through DraftGo Launcher no longer drops OAuth MCP `auth` blocks or pollutes unrelated YAML keys; guard tests added via `tests/hermes_roundtrip.rs`.
...
- **Lightweight Mode**: Added a tray-only mode that destroys the main window and keeps DraftGo Launcher running from the system tray, with the window recreated when users reopen it.
...
- **Skills Discovery & Batch Updates**: Added SHA-256 based skill update detection, per-skill and batch update actions, a storage-location toggle between DraftGo Launcher and `~/.agents/skills`, and public `skills.sh` search integration.
...
- **Skill Auto-Backup**: Skill files are automatically backed up to `~/.draftgo-launcher/skill-backups/` before uninstall, with metadata preserved in `meta.json`; old backups pruned to keep at most 20
...
- **OpenClaw Integration**: Full management support for OpenClaw as the fifth application in DraftGo Launcher, including provider switching, configuration panels (Env / Tools / Agents Defaults), Workspace file management (HEARTBEAT / BOOTSTRAP / BOOT), daily memory files, and additive overlay mode
...
- **Crash Logging** - Panic hook captures crash info to `~/.draftgo-launcher/crash.log` with full stack traces (#562)
...
- **Windows Title** - Display "DraftGo Launcher" instead of default "Tauri app" in window title
...
- **Import/Export** - Restrict SQL import to DraftGo Launcher exported backups only; refresh providers immediately after import
...
- **ArchLinux installation support** (#259) - Added AUR installation via `paru -S draftgo-launcher-bin`
...
- **Deep link support** - Import Gemini providers via `draftgo://` protocol
...
#### Deep Link Protocol (draftgo://)

- **Protocol registration** - `draftgo://` URL scheme for one-click imports
...
- **Standardized release artifact naming** - All platform releases now use consistent version-tagged filenames:
  - macOS: `DraftGo-Launcher-v{version}-macOS.tar.gz` / `.zip`
  - Windows: `DraftGo-Launcher-v{version}-Windows.msi` / `-Portable.zip`
  - Linux: `DraftGo-Launcher-v{version}-Linux.AppImage` / `.deb`
...
#### Backup on v1→v2 Migration (draftgo-launcher internal config)

- When the app detects an old v1 config structure at `~/.draftgo-launcher/config.json`, it now creates a timestamped backup before writing the new v2 structure.
- Backup location: `~/.draftgo-launcher/config.v1.backup.<timestamp>.json`
- This only concerns draftgo-launcher's own metadata file; your actual provider files under `~/.claude/` and `~/.codex/` are untouched.
