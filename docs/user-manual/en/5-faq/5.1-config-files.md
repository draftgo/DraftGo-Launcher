...
## DraftGo Launcher Data Storage

### Storage Directory

Default location: `~/.draftgo-launcher/`
...
```
~/.draftgo-launcher/
├── draftgo-launcher.db      # SQLite database (SSOT)
...
`draftgo-launcher.db` is a SQLite database that stores:
...
Hermes uses YAML configuration. DraftGo Launcher writes MCP servers to `mcp_servers`, writes editable provider entries to `custom_providers`, reads read-only entries from Hermes' `providers` dict, and updates `model.provider` / `model.default` when switching providers.
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to DraftGo Launcher's "providers") |
...
DraftGo Launcher's priority when modifying configurations:

1. **DraftGo Launcher Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by DraftGo Launcher)
- DraftGo Launcher's `settings.json`

### Not Recommended to Edit Manually

- `draftgo-launcher.db` database file
- Backup files
...
DraftGo Launcher v3.7.0 migrated from JSON files to SQLite:
