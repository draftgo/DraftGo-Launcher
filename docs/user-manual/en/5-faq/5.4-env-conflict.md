...
DraftGo Launcher automatically detects conflicts between system environment variables and app configurations, preventing configurations from being unexpectedly overridden.
...
Found X environment variables that may conflict with DraftGo Launcher configuration
[Expand] [Dismiss]
```
...
3. DraftGo Launcher will automatically back up and remove the selected variables

### Automatic Backup

A backup is automatically created before removal:

- Backup location: `~/.draftgo-launcher/env-backups/`
...
If you prefer not to use DraftGo Launcher to remove variables, you can handle them manually:
...
- DraftGo Launcher provider configurations being overridden
- API requests being sent to the wrong endpoint
- Using the wrong API key

## Best Practices

1. **Use DraftGo Launcher to manage configurations**: Avoid setting API keys in system environment variables
...
1. Find the backup file: `~/.draftgo-launcher/env-backups/`
