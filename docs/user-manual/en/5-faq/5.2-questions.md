...
DraftGo Launcher for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/draftgo/DraftGo-Launcher/releases).
...
2. Add DraftGo Launcher to your antivirus software's whitelist
...
chmod +x DraftGo-Launcher-*.AppImage

# If it still fails, try
./DraftGo-Launcher-*.AppImage --no-sandbox
```
...
**Solution**: Use the dedicated `DRAFTGO_LAUNCHER_GDK_BACKEND` environment variable to switch back to native Wayland (it is read before GTK init, and the hook never overrides it):

```bash
DRAFTGO_LAUNCHER_GDK_BACKEND=wayland ./DraftGo-Launcher-*.AppImage
```

- When launching from a desktop icon, add it to the `.desktop` `Exec=` line (e.g. `env DRAFTGO_LAUNCHER_GDK_BACKEND=wayland /path/to/AppImage`) or set it in your session environment — otherwise an icon launch won't see the variable.
- The variable is generic: on tiling Wayland compositors (sway/Hyprland) where clicks don't register, set `DRAFTGO_LAUNCHER_GDK_BACKEND=x11` instead.
...
**Solutions**:
1. Check if the `~/.draftgo-launcher/` directory exists
2. Restore from backup: `~/.draftgo-launcher/backups/`
...
**Solutions**:
1. Confirm the file was exported by DraftGo Launcher
...
**Windows**:
- Check taskbar settings to ensure the DraftGo Launcher icon is not hidden
...
2. Restart the app
3. Delete `~/.draftgo-launcher/settings.json` to reset settings
...
3. If using Homebrew: `brew upgrade --cask draftgo-launcher`

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and DraftGo Launcher runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, DraftGo Launcher covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `draftgo://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/draftgo/DraftGo-Launcher/issues)
2. Search for similar issues
3. If none found, create a new Issue
4. Provide the following information:
   - Operating system and version
   - DraftGo Launcher version
   - Problem description and reproduction steps
   - Error messages (if any)

### Log Files

DraftGo Launcher stores logs under its application configuration directory. By default this is `.draftgo-launcher` in your home directory; if you changed the configuration directory in Advanced Settings, use that custom location instead.

When submitting an Issue, attach the files relevant to the problem:

- General, network, or proxy errors: `~/.draftgo-launcher/logs/draftgo-launcher.log` and its rotated files
- Application crashes: `~/.draftgo-launcher/crash.log`, `crash.log.1`, and `crash.log.2`
- The default Windows location is `C:\Users\<username>\.draftgo-launcher\...`
