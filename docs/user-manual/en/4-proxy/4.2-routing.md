...
App routing means letting DraftGo Launcher route a specific application's API requests through the local routing service.
...
When routing is enabled, DraftGo Launcher modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, DraftGo Launcher will:
...
Before enabling routing, DraftGo Launcher backs up the original configuration and restores it when disabled.
