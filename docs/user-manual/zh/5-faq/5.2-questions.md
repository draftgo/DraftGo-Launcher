...
DraftGo Launcher macOS 版本已通过 Apple 代码签名和公证，可直接下载安装，无需额外操作。如遇问题，请尝试从 [Releases 页面](https://github.com/draftgo/DraftGo-Launcher/releases) 下载最新版本。
...
2. 将 DraftGo Launcher 添加到杀毒软件白名单
...
**解决方法**：用专用环境变量 `DRAFTGO_LAUNCHER_GDK_BACKEND` 切回原生 Wayland（该开关在 GTK 初始化前生效，钩子不会覆盖它）：

```bash
DRAFTGO_LAUNCHER_GDK_BACKEND=wayland ./DraftGo-Launcher-*.AppImage
```

- 从桌面图标启动时，把它写进 `.desktop` 的 `Exec=` 行（如 `env DRAFTGO_LAUNCHER_GDK_BACKEND=wayland /path/to/AppImage`），或在会话环境中设置，否则图标启动读不到该变量。
- 该变量是通用的：在 tiling Wayland 合成器（sway/Hyprland）下若反而出现点击失效，可设 `DRAFTGO_LAUNCHER_GDK_BACKEND=x11`。
...
**解决方法**：
1. 检查 `~/.draftgo-launcher/` 目录是否存在
2. 从备份恢复：`~/.draftgo-launcher/backups/`
3. 或从之前导出的配置文件导入
...
**解决方法**：
1. 确认文件是 DraftGo Launcher 导出的 SQL 备份文件
2. 检查文件内容是否完整
3. 尝试用文本编辑器打开检查格式
...
**Windows**：
- 检查任务栏设置，确保 DraftGo Launcher 图标未被隐藏
...
1. 尝试切换主题（浅色/深色）
2. 重启应用
3. 删除 `~/.draftgo-launcher/settings.json` 重置设置
...
1. 检查网络连接
2. 手动下载最新版本安装
3. 如使用 Homebrew：`brew upgrade --cask draftgo-launcher`

## 轻量模式

### 如何进入轻量模式？

从系统托盘菜单切换"轻量模式"。主窗口关闭，DraftGo Launcher 仅作为托盘应用运行。再次切换或点击"打开主界面"即可退出。
...
可以。DraftGo Launcher v3.13.0 起会覆盖所有窗口重新显示路径（正常启动、深链接、单例激活、托盘 `show_main` 以及轻量模式返程），点击 `draftgo://` 链接会**按需重建**主窗口并显示导入确认对话框。第一次打开会比普通状态略慢（需要重建窗口），但后续切换恢复正常速度。
...
1. 访问 [GitHub Issues](https://github.com/draftgo/DraftGo-Launcher/issues)
2. 搜索是否有类似问题
3. 如果没有，创建新 Issue
4. 提供以下信息：
   - 操作系统和版本
...
DraftGo Launcher 的日志位于应用配置目录中。默认配置目录是用户主目录下的 `.draftgo-launcher`；如果你在高级设置中修改过配置目录，请以自定义目录为准。

提交 Issue 时，请按问题类型附上：

- 普通错误、网络或代理问题：`~/.draftgo-launcher/logs/` 中的 `draftgo-launcher.log` 及其轮转文件
- 应用崩溃：`~/.draftgo-launcher/crash.log` 及 `crash.log.1`、`crash.log.2`
- Windows 默认路径为 `C:\Users\<用户名>\.draftgo-launcher\...`
