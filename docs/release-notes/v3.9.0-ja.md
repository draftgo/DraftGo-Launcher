...
DraftGo Launcher v3.9.0 は v3.9 ベータ（`3.9.0-1`、`3.9.0-2`、`3.9.0-3`）の安定版です。
...
- SQL インポートは DraftGo Launcher がエクスポートしたバックアップのみに制限されます（安全性のため）。
- プロキシのテイクオーバーは CLI の live 設定を変更します。DraftGo Launcher はリダイレクト前に live 設定をバックアップします。元に戻す場合はテイクオーバー無効化/プロキシ停止を行い、必要に応じてバックアップから復元してください。
...
[Releases](https://github.com/draftgo/DraftGo-Launcher/releases/latest) から該当するバージョンをダウンロードしてください。
...
| ファイル                                | 説明                                         |
| --------------------------------------- | -------------------------------------------- |
| `DraftGo-Launcher-v3.9.0-Windows.msi`          | **推奨** - MSI インストーラー、自動更新対応  |
| `DraftGo-Launcher-v3.9.0-Windows-Portable.zip` | ポータブル版、インストール不要               |

### macOS

| ファイル                        | 説明                                                              |
| ------------------------------- | ----------------------------------------------------------------- |
| `DraftGo-Launcher-v3.9.0-macOS.zip`    | **推奨** - 解凍して Applications へドラッグ、Universal Binary     |
| `DraftGo-Launcher-v3.9.0-macOS.tar.gz` | Homebrew インストールおよび自動更新用                             |
...
| サンドボックスで実行したい場合          | `.flatpak`  | `flatpak install DraftGo-Launcher-*.flatpak`                                          |
