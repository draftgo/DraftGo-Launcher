...
- **Move back and forth between the official subscription and DeepSeek instead of choosing one**: `auth.json` and `config.toml` are single-slot files — Codex itself cannot hold a second set of credentials. A vendor's one-click script rewrites that configuration to be its own, whereas DraftGo Launcher snapshots and restores it per provider — which is the most practical difference between the two approaches; see [the comparison below](#importing-through-draftgo-launcher-vs-running-the-official-script).
...
Codex reads model capabilities from a catalog file, and DraftGo Launcher previously generated that catalog from a neutral template for every provider — which is right for an aggregator, but strips the capabilities a vendor's own integration depends on. Now, for any vendor whose official catalog is bundled with the app, that vendor's own file is mirrored directly.
...
Codex sorts its resume list into drawers by the `model_provider` recorded in each session. Every third-party Codex provider DraftGo Launcher creates — DeepSeek, Kimi, an aggregator, it makes no difference — writes the same identifier `custom`, so however you switch among them, `codex resume` keeps showing the full history. On its first launch, DraftGo Launcher also performs a one-time migration that folds known per-vendor buckets (including the `deepseek` one the official script writes) into this shared bucket, backing the original files up to `~/.draftgo-launcher/backups/` first.

**There is a clear boundary here**: that migration runs once, on DraftGo Launcher's first launch. **If you install DraftGo Launcher first and only later run the official script**, those `deepseek`-tagged sessions will not be folded in — they stay in their own drawer. Separately, a provider identifier you wrote by hand that is not on the known list is deliberately left alone.

### Official-Subscription Sessions Are Already Interleaved With Third-Party Ones in DraftGo Launcher

DraftGo Launcher's **Sessions panel scans the session directory directly and does not read `model_provider`**, so Codex sessions produced during official-subscription use have always been in the same list as third-party ones — searchable, resumable, deletable — **with no toggle required**.
...
>
> **First, everything above applies to the Codex directory DraftGo Launcher points at.** That is `~/.codex` by default and can be changed in Settings. **DraftGo Launcher does not read the `CODEX_HOME` environment variable** — if you use that variable to point Codex somewhere else, DraftGo Launcher cannot see those sessions, and provider switches will be written into a directory the CLI is not using. To change the directory, use DraftGo Launcher's own config-directory setting.
>
...
A companion change consolidates proxy state and takeover state onto a single query layer — there had been a second parallel set of hooks over the same commands with zero callers, whose query keys never had an observer, so every invalidation aimed at them was a no-op. The query key strings are unchanged character for character, and the surviving hook keeps its existing polling behavior. ([#5916](https://github.com/draftgo/DraftGo-Launcher/pull/5916), [#5928](https://github.com/draftgo/DraftGo-Launcher/pull/5928))
...
The four repriced models work in the opposite direction: the backfill only touches zero-cost rows, so requests already recorded keep the old price and only new requests bill at the new one — historical and new spend for the same model will not match. Both paths protect your own pricing: the repair only changes rows still at the original built-in value, while manual edits, models.dev sync values, and deletion tombstones in `~/.draftgo-launcher/model-pricing.json` are replayed after seeding and repair and always win.
...
The mirrored catalog declares a minimum Codex client version of 0.144.0, and **DraftGo Launcher does not verify this itself** — the freeform `apply_patch` registration it carries requires that version or newer. Separately, the generated catalog file grows to roughly 75 KB (for the two mirrored models), because each entry carries the full harness text.
...
**Third-party provider routing**: when the DraftGo Launcher local proxy converts and forwards Codex, Claude Desktop, or Grok Build requests to a third-party provider, each provider has different constraints on billing, compliance, and data retention. Please read the target provider's terms of service before use.

By enabling these features, users accept the associated risks. DraftGo Launcher is not responsible for any account restriction, warning, or service suspension resulting from their use.
...
- [#5677](https://github.com/draftgo/DraftGo-Launcher/pull/5677): finishing off Grok Build's proxy takeover and deep-link integration — the backend field, session identity, the failover tab, and environment-variable detection, plus a fix for the credential leak in config previews along the way. Thanks to @YUZHEthefool. This is the broadest single piece of work in the release.
- [#5951](https://github.com/draftgo/DraftGo-Launcher/pull/5951): the Claude Desktop double-count fix. Thanks to @Komikawayi. Pinpointing which change in v3.18.0 made all three dedup defenses fail at once was the most patient piece of investigation in this release.
- [#5916](https://github.com/draftgo/DraftGo-Launcher/pull/5916), [#5928](https://github.com/draftgo/DraftGo-Launcher/pull/5928): removing 3,166 lines of code with no callers and the duplicate proxy query layer. Thanks to @SaladDay.
- [#5943](https://github.com/draftgo/DraftGo-Launcher/pull/5943): completing the Traditional Chinese tool-management strings and adding a test that prevents locale drift. Thanks to @yovinchen.
- [#5960](https://github.com/draftgo/DraftGo-Launcher/pull/5960): completing the 9 strings that showed Simplified Chinese in every language. Thanks to @mhy1227.

### Issue Reports

Thanks to @Alaric-L for reporting in [#5938](https://github.com/draftgo/DraftGo-Launcher/issues/5938) that every Claude Desktop request produced an extra log row sourced from `session_log`, causing tokens to be counted twice — that report pinpointed the data source, and the most important usage fix in this release was located directly from it.
...
| File                                     | Description                                      |
| ---------------------------------------- | ------------------------------------------------ |
| `DraftGo-Launcher-v3.19.1-Windows.msi`          | **Recommended** - MSI installer with auto-update |
| `DraftGo-Launcher-v3.19.1-Windows-Portable.zip` | Portable build, unzip and run                    |
...
| File                             | Description                                           |
| -------------------------------- | ----------------------------------------------------- |
| `DraftGo-Launcher-v3.19.1-macOS.dmg`    | **Recommended** - DMG installer, drag to Applications |
| `DraftGo-Launcher-v3.19.1-macOS.zip`    | Unzip and drag to Applications, Universal Binary      |
| `DraftGo-Launcher-v3.19.1-macOS.tar.gz` | For Homebrew install and auto-update                  |
...
- `DraftGo-Launcher-v3.19.1-Linux-x86_64.AppImage` / `.deb` / `.rpm`
- `DraftGo-Launcher-v3.19.1-Linux-arm64.AppImage` / `.deb` / `.rpm`
