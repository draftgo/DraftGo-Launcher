...
- **Skills 发现与批量更新**：基于 SHA-256 内容哈希的更新检测、单项和"全部更新"批量操作、`skills.sh` 表搜索集成，以及 DraftGo Launcher 与 `~/.agents/skills` 的存储位置切换
...
新增仅托盘运行模式，显著降低 DraftGo Launcher 空闲时的桌面占用。
...
- **存储位置切换**：在 DraftGo Launcher 存储和 `~/.agents/skills` 之间切换而不丢失 skill 状态
...
新安装用户首次打开时显示一次性欢迎对话框，引导了解 DraftGo Launcher 工作流程。
...
| 文件                                     | 说明                                |
| ---------------------------------------- | ----------------------------------- |
| `DraftGo-Launcher-v3.13.0-Windows.msi`          | **推荐** - MSI 安装包，支持自动更新 |
| `DraftGo-Launcher-v3.13.0-Windows-Portable.zip` | 便携版，解压即用，不写入注册表      |

### macOS

| 文件                             | 说明                                          |
| -------------------------------- | --------------------------------------------- |
| `DraftGo-Launcher-v3.13.0-macOS.dmg`    | **推荐** - DMG 安装包，拖入 Applications 即可 |
| `DraftGo-Launcher-v3.13.0-macOS.zip`    | 解压后拖入 Applications，Universal Binary     |
| `DraftGo-Launcher-v3.13.0-macOS.tar.gz` | 用于 Homebrew 安装和自动更新                  |
