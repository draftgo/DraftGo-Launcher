...
export interface Provider {
  ...
  // 新增：是否为商业合作伙伴
  isPartner?: boolean;
  // 可选：供应商元数据（仅存于 ~/.draftgo-launcher/config.json，不写入 live 配置）
  meta?: ProviderMeta;
  // 图标配置
...
// Skill 存储位置
export type SkillStorageLocation = "draftgo_launcher" | "unified";
...
// 存储在本地 ~/.draftgo-launcher/settings.json，不随数据库同步
export interface Settings {
  ...
  // Skill 存储位置：draftgo_launcher（默认）或 unified（~/.agents/skills/）
