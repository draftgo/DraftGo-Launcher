...
This directory contains the Flatpak manifest (`com.draftgo.launcher`) for DraftGo Launcher, used to convert the generated `.deb` artifact into an installable `.flatpak` package via CI or local builds.
...
```bash
cp "$(find src-tauri/target/release/bundle -name '*.deb' | head -n 1)" flatpak/draftgo-launcher.deb
```

3) Build the local Flatpak repository and export the `.flatpak`:

```bash
flatpak-builder --force-clean --user --disable-cache --repo flatpak-repo flatpak-build flatpak/com.draftgo.launcher.yml
flatpak build-bundle --runtime-repo=https://flathub.org/repo/flathub.flatpakrepo flatpak-repo DraftGo-Launcher-Linux.flatpak com.draftgo.launcher
```

4) Install and run:

```bash
flatpak install --user ./DraftGo-Launcher-Linux.flatpak
flatpak run com.draftgo.launcher
```
...
If you prefer minimal permissions (e.g., for Flathub submission or security concerns), you can replace `--filesystem=home` in `flatpak/com.draftgo.launcher.yml` with more precise grants:

```yaml
  - --filesystem=~/.draftgo-launcher:create
...
If you plan to publish on Flathub or want stricter permission control, adjust the `finish-args` in `flatpak/com.draftgo.launcher.yml` accordingly.
