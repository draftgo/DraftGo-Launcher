...
### 📦 Quellcode & Releases: **[github.com/draftgo/DraftGo-Launcher](https://github.com/draftgo/DraftGo-Launcher)**

[English](README.md) | [中文](README_ZH.md) | [日本語](README_JA.md) | Deutsch | [Changelog](CHANGELOG.md)

</div>

## Warum DraftGo Launcher?
...
- **Deep Link** (`draftgo://`) — Importieren Sie Anbieter, MCP-Server, Prompts und Skills per URL
...
<details>
<summary><strong>Welche KI-Werkzeuge unterstützt DraftGo Launcher?</strong></summary>

DraftGo Launcher unterstützt acht Werkzeuge: **Claude Code**, **Claude Desktop**, **Codex**, **Gemini CLI**, **Grok Build**, **OpenCode**, **OpenClaw** und **Hermes**. Jedes Werkzeug verfügt über dedizierte Anbieter-Presets und Konfigurationsverwaltung.
...
DraftGo Launcher für macOS ist von Apple code-signiert und notarisiert. Sie können es direkt herunterladen und installieren — es sind keine zusätzlichen Schritte erforderlich. Wir empfehlen die Verwendung des `.dmg`-Installationsprogramms.
...
- **Datenbank**: `~/.draftgo-launcher/draftgo-launcher.db` (SQLite — Anbieter, MCP, Prompts, Skills)
- **Lokale Einstellungen**: `~/.draftgo-launcher/settings.json` (gerätebezogene UI-Einstellungen)
- **Backups**: `~/.draftgo-launcher/backups/` (automatisch rotiert, behält die 10 neuesten)
- **Skills**: `~/.draftgo-launcher/skills/` (standardmäßig per Symlink mit den entsprechenden Apps verbunden)
- **Skill-Backups**: `~/.draftgo-launcher/skill-backups/` (vor der Deinstallation automatisch erstellt, behält die 20 neuesten)
...
Wenn Sie über ein Desktop-Symbol starten, fügen Sie es der `Exec=`-Zeile der `.desktop`-Datei hinzu (z. B. `env DRAFTGO_LAUNCHER_GDK_BACKEND=wayland /pfad/zum/AppImage`) oder setzen Sie es in Ihrer Sitzungsumgebung. Die Variable ist generisch: Auf Tiling-Wayland-Compositors (sway/Hyprland), bei denen Klicks nicht reagieren, versuchen Sie umgekehrt `DRAFTGO_LAUNCHER_GDK_BACKEND=x11`. Bleibt sie ungesetzt, bleibt das Standardverhalten erhalten.
...
Laden Sie das neueste Installationsprogramm `DraftGo-Launcher-v{version}-Windows.msi` oder die portable Version `DraftGo-Launcher-v{version}-Windows-Portable.zip` von der Seite [Releases](../../releases) herunter.
...
Laden Sie `DraftGo-Launcher-v{version}-macOS.dmg` (empfohlen) oder `.zip` von der Seite [Releases](../../releases) herunter.

> **Hinweis**: DraftGo Launcher für macOS ist von Apple code-signiert und notarisiert. Sie können es direkt installieren und öffnen.
...
- `DraftGo-Launcher-v{version}-Linux.AppImage` (universell)
...
- **SSOT** (Single Source of Truth): Alle Daten werden in `~/.draftgo-launcher/draftgo-launcher.db` (SQLite) gespeichert
