...
use draftgo_launcher_lib::{update_settings, AppSettings, AppState, Database, MultiAppConfig};

/// 为测试设置隔离的 HOME 目录，避免污染真实用户数据。
pub fn ensure_test_home() -> &'static Path {
    static HOME: OnceLock<PathBuf> = OnceLock::new();
    HOME.get_or_init(|| {
        let base = std::env::temp_dir().join("draftgo-launcher-test-home");
        if base.exists() {
        ...
        // 用 DRAFTGO_LAUNCHER_TEST_HOME 显式覆盖，以确保测试不会污染真实用户目录。
        std::env::set_var("DRAFTGO_LAUNCHER_TEST_HOME", &base);
