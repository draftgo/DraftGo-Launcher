...
fn sync_codex_provider_writes_config_without_touching_auth() {
    ...
    let auth_path = draftgo_launcher_lib::get_codex_auth_path();
...
requires_openai_auth = true
"#;
    draftgo_launcher_lib::write_codex_live_atomic(&legacy_auth, Some(legacy_config))
...
fn sync_enabled_to_codex_writes_enabled_servers() {
    ...
    let path = draftgo_launcher_lib::get_codex_config_path();
    ...
    draftgo_launcher_lib::sync_enabled_to_codex(&config).expect("sync codex");
...
fn sync_enabled_to_codex_returns_error_on_invalid_toml() {
    ...
    let err = draftgo_launcher_lib::sync_enabled_to_codex(&config).expect_err("sync should fail");
    match err {
        draftgo_launcher_lib::AppError::Toml { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "path should reference config.toml"
            );
        }
        draftgo_launcher_lib::AppError::McpValidation(msg) => {
            assert!(
...
fn sync_single_server_to_codex_fails_closed_on_invalid_toml() {
    ...
    let err = draftgo_launcher_lib::sync_single_server_to_codex(
        &config,
        "srv",
...
fn sync_codex_provider_missing_auth_returns_error() {
    ...
    match err {
        draftgo_launcher_lib::AppError::Config(msg) => {
    ...
    assert!(
        !draftgo_launcher_lib::get_codex_config_path().exists(),
...
"#;
    ...
    draftgo_launcher_lib::write_codex_live_atomic(&auth, Some(config_text))
    ...
    let stored_auth: serde_json::Value =
        draftgo_launcher_lib::read_json_file(&auth_path).expect("read auth");
...
"#;
    ...
    let err = draftgo_launcher_lib::write_codex_live_atomic(&auth, Some(config_text))
        .expect_err("config write should fail when target is directory");
    match err {
        draftgo_launcher_lib::AppError::Io { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "io error path should point to config.toml"
            );
        }
        draftgo_launcher_lib::AppError::IoContext { context, .. } => {
            assert!(
...
"#,
    ...
    let mut config = MultiAppConfig::default();
    let changed = draftgo_launcher_lib::import_from_codex(&mut config).expect("import codex");
...
"#,
    ...
    config.mcp.servers.as_mut().unwrap().insert(
        "existing".to_string(),
        draftgo_launcher_lib::McpServer {
            ...
            apps: draftgo_launcher_lib::McpApps {
                claude: false,
                codex: false, // 初始未启用
                gemini: false,
                grokbuild: false,
...
fn sync_claude_enabled_mcp_projects_to_user_config() {
    ...
    draftgo_launcher_lib::sync_enabled_to_claude(&config).expect("sync Claude MCP");

    let claude_path = draftgo_launcher_lib::get_claude_mcp_path();
...
fn create_backup_skips_missing_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_path = home.join(".draftgo-launcher").join("config.json");
...
fn create_backup_generates_snapshot_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_dir = home.join(".draftgo-launcher");
...
fn sync_gemini_packycode_sets_security_selected_type() {
    ...
    // security field is written to ~/.gemini/settings.json, not ~/.draftgo-launcher/settings.json
...
fn export_sql_returns_error_for_invalid_path() {
    ...
    let invalid_parent = if cfg!(windows) {
        std::env::temp_dir().join("draftgo-launcher-test-invalid<>dir")
    } else {
...
#[test]
fn import_sql_rejects_non_draftgo_launcher_backup() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
    ...
    let import_path = home.join("not-draftgo-launcher.sql");
    fs::write(&import_path, "CREATE TABLE x (id INTEGER);").expect("write import sql");

    let err = state
        .db
        .import_sql(&import_path)
        .expect_err("non-draftgo-launcher sql should be rejected");
...
#[test]
fn import_sql_accepts_draftgo_launcher_exported_backup() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
    ...
    let state = create_test_state_with_config(&config).expect("create test state");
    let export_path = home.join("draftgo-launcher-export.sql");
    state
        .db
        .export_sql(&export_path)
        .expect("export should succeed");
