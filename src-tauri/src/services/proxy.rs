...
///
/// 原因：接管模式下 `*_MODEL` 必须由 DraftGo Launcher 写成稳定的 Claude 角色别名，
...
mod tests {
    ...
    impl TempHome {
        fn new() -> Self {
            ...
            let original_test_home = env::var("DRAFTGO_LAUNCHER_TEST_HOME").ok();
...
"#;
    ...
    fn apply_codex_proxy_toml_config_routes_builtin_official_with_native_auth() {
        ...
        let route_id = crate::codex_config::DRAFTGO_LAUNCHER_CODEX_OFFICIAL_PROXY_PROVIDER_ID;
...
"#;
        ...
        assert!(
            catalog_path.exists(),
            "draftgo-launcher-model-catalog.json must be created on provider switch"
        );
...
"#;
    ...
    async fn codex_restore_from_backup_preserves_model_catalog_pointer() {
        ...
        // Pre-takeover Live state: config.toml points at the draftgo-launcher generated
        ...
        assert!(
            ...
            "restored pointer must still reference the draftgo-launcher generated catalog file"
        );
    }
    ...
    async fn codex_restore_from_backup_projects_inline_model_catalog() {
        ...
        assert!(
            catalog_path.exists(),
            "restore must generate the draftgo-launcher catalog file on disk"
        );
    ...
    async fn codex_restore_empty_auth_backup_still_projects_inline_catalog() {
        ...
        assert!(
            ...
            "empty-auth restore must generate the draftgo-launcher catalog file"
        );
        assert!(
