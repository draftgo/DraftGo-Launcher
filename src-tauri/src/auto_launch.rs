...
/// 将 `/path/to/DraftGo Launcher.app/Contents/MacOS/DraftGo Launcher` 转换为 `/path/to/DraftGo Launcher.app`
#[cfg(target_os = "macos")]
...
mod tests {
    ...
    fn test_get_macos_app_bundle_path_with_spaces() {
        ...
        assert_eq!(
            result,
            Some(std::path::PathBuf::from(
                "/Users/test/My Apps/DraftGo Launcher.app"
            ))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_not_in_bundle() {
        let exe_path = std::path::Path::new("/usr/local/bin/draftgo-launcher");
        let result = get_macos_app_bundle_path(exe_path);
    ...
    fn test_get_macos_app_bundle_path_dev_build() {
        ...
        let exe_path = std::path::Path::new("/Users/dev/project/target/debug/draftgo-launcher");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(result, None);
    }
}
