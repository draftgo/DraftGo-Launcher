...
///
/// Input (from DraftGo Launcher UI / database):
/// ```json
...
/// `providers:` dict. DraftGo Launcher treats those as read-only — writes have to
...
/// - `custom_providers:` list entries (writable by DraftGo Launcher)
...
/// keeping DraftGo Launcher free to edit it. Models are denormalized from the YAML
...
/// list — i.e. it is a read-only overlay DraftGo Launcher must not touch.
...
pub fn set_provider(
        ...
        .find(|p| p.get("name").and_then(|n| n.as_str()) == Some(name))
    {
        ...
        // this merge, a DraftGo Launcher edit to an unrelated field would silently
...
// DraftGo Launcher fills that gap by reading/writing the whole file as a markdown
...
mod tests {
    ...
    ///
    /// Saves and restores `DRAFTGO_LAUNCHER_TEST_HOME` to avoid interfering with
    ...
    fn with_test_home<T>(test_fn: impl FnOnce() -> T) -> T {
        ...
        std::env::set_var("DRAFTGO_LAUNCHER_TEST_HOME", tmp.path());
...
";
    ...
    fn set_provider_preserves_unknown_fields_on_update() {
        ...
        // Hermes Web UI; a later DraftGo Launcher edit must not strip them — set_provider
...
";
    ...
    fn set_memory_enabled_preserves_other_fields() {
        ...
        // a DraftGo Launcher toggle would silently wipe those fields.
        with_test_home(|| {
            let yaml = "\
memory:
