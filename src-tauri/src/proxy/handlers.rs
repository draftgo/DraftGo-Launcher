...
/// catalog with a top-level `models` field.  Return the draftgo-launcher–managed model
...
/// draftgo-launcher–owned `model_catalog_json`, using the same path ownership rules as
...
pub async fn handle_models() -> Result<Json<Value>, ProxyError> {
        ...
        active_catalog_path.as_ref().filter(|path| path.exists())
    {
        ...
        if active_catalog_path.is_none() {
            log::debug!(
                "[models] stale guard: catalog not served (model_catalog_json not set to draftgo-launcher catalog)"
            );
        }
        json!({"models": []})
    };
...
fn codex_proxy_error_json(
    ...
    let message = if upstream_status == Some(413) {
        ...
        // 避免「以为是 DraftGo Launcher 封装了 nginx / 是本地代理的锅」这种反复出现的误解。
        format!(
            concat!(
                ...
                "provider's server-side limit, not a DraftGo Launcher limit. ",
        ...
        let status_fragment = upstream_status
            ...
            .unwrap_or_default();
        format!(
            "DraftGo Launcher local proxy failed while handling Codex endpoint {endpoint}. Provider: {provider_name}; model: {request_model}{status_fragment}; cause: {cause}"
        )
    };
...
fn codex_proxy_error_code(error: &ProxyError) -> &'static str {
    match error {
        ProxyError::ForwardFailed(_) => "draftgo_launcher_forward_failed",
        ProxyError::Timeout(_) | ProxyError::StreamIdleTimeout(_) => "draftgo_launcher_timeout",
        ProxyError::NoAvailableProvider => "draftgo_launcher_no_available_provider",
        ProxyError::AllProvidersCircuitOpen => "draftgo_launcher_all_providers_circuit_open",
        ProxyError::NoProvidersConfigured => "draftgo_launcher_no_providers_configured",
        ProxyError::MaxRetriesExceeded => "draftgo_launcher_max_retries_exceeded",
        ProxyError::ProviderUnhealthy(_) => "draftgo_launcher_provider_unhealthy",
        ProxyError::ConfigError(_) => "draftgo_launcher_config_error",
        ProxyError::TransformError(_) => "draftgo_launcher_transform_error",
        ProxyError::InvalidRequest(_) => "draftgo_launcher_invalid_request",
        ProxyError::AuthError(_) => "draftgo_launcher_auth_error",
        ProxyError::UpstreamError { .. } => "draftgo_launcher_upstream_error",
        ProxyError::DatabaseError(_) => "draftgo_launcher_database_error",
        ProxyError::Internal(_) => "draftgo_launcher_internal_error",
        ProxyError::AlreadyRunning
        | ProxyError::NotRunning
        | ProxyError::BindFailed(_)
        | ProxyError::StopTimeout
        | ProxyError::StopFailed(_) => "draftgo_launcher_proxy_error",
    }
}
...
data: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"message\"}}\n\n";
    ...
    fn codex_proxy_forward_error_includes_context_and_cause() {
        ...
        assert!(message.contains("DraftGo Launcher local proxy failed"));
        ...
        assert_eq!(body["error"]["code"], "draftgo_launcher_forward_failed");
