...
fn handle_deeplink_url(
    ...
    if !url_str.starts_with("draftgo://") {
        return false;
    }
...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.draftgo-launcher/crash.log）
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（输出到 <app_config_dir>/logs/draftgo-launcher.log）
            {
                    ...
                    tauri_plugin_log::Builder::default()
                        ...
                        .targets([
                            ...
                            Target::new(TargetKind::Folder {
                                path: log_dir,
                                file_name: Some("draftgo-launcher".into()),
                            }),
                        ])
                ...
                log::info!("=== DraftGo Launcher v{} started ===", env!("CARGO_PKG_VERSION"));
            }
            ...
            let db_path = app_config_dir.join("draftgo-launcher.db");
            ...
            //
            // 捕获首次运行快照：所有全新装用户都会看到欢迎弹窗介绍 DraftGo Launcher 的工作方式。
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    // tauri-plugin-deep-link writes to: ~/.local/share/com.draftgo.launcher/applications/draftgo-launcher-handler.desktop
                    // Only register if .desktop file doesn't exist to avoid overwriting user customizations
                    let should_register = app
                        .path()
                        .data_dir()
                        .map(|d| !d.join("applications/draftgo-launcher-handler.desktop").exists())
                        .unwrap_or(true);
                ...
                move |event| {
                    ...
                    for (i, url) in urls.iter().enumerate() {
                        ...
                        if handle_deeplink_url(&app_handle, url_str, true, "on_open_url") {
                            break; // Process only first draftgo:// URL
                        }
                    }
                }
            });
            ...
            let mut tray_builder = TrayIconBuilder::with_id(tray::TRAY_ID)
                .tooltip("DraftGo Launcher") // 鼠标悬停提示
    ...
    app.run(|app_handle, event| {
        ...
        #[cfg(target_os = "macos")]
        {
            match event {
                ...
                // 处理通过自定义 URL 协议触发的打开事件（例如 draftgo://...）
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        format!(
            "从旧版本迁移配置时发生错误：\n\n{error}\n\n\
            您的数据尚未丢失，旧配置文件仍然保留。\n\
            建议回退到旧版本 DraftGo Launcher 以保护数据。\n\n\
            点击「重试」重新尝试迁移\n\
        ...
        format!(
            ...
            Consider rolling back to an older DraftGo Launcher version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        format!(
            ...
            建议：\n\
            1) 先备份整个配置目录（包含 draftgo-launcher.db）\n\
        ...
        format!(
            ...
            Suggestions:\n\
            1) Back up the entire config directory (including draftgo-launcher.db)\n\
            2) If you see “database version is newer”, please upgrade DraftGo Launcher\n\
