...
pub async fn check_for_updates(handle: AppHandle) -> Result<bool, String> {
    handle
        .opener()
        .open_url(
            "https://github.com/draftgo/DraftGo-Launcher/releases/latest",
            None::<String>,
        )
...
async fn fetch_github_latest_version(client: &reqwest::Client, repo: &str) -> Option<String> {
    ...
    match client
        .get(&url)
        .header("User-Agent", "draftgo-launcher")
...
///
/// 判定顺序（命中即返回）：
/// ① Hermes → `<bin_path 绝对> update`;Hermes CLI 自己知道安装环境,避免 draftgo-launcher
...
pub(crate) fn launch_terminal_running(command_line: &str, label: &str) -> Result<(), String> {
    ...
    let (script_file, script_content) = {
        let file = temp_dir.join(format!("draftgo_launcher_{}_{}.sh", label, pid));
        let content = format!(
            r#"#!/usr/bin/env sh
trap 'rm -f "{script_path}"' EXIT
echo "[draftgo-launcher] Starting: {label}"
echo ""
{cmd}
echo ""
echo "[draftgo-launcher] Command exited. Press Enter to close."
read -r _
"#,
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let bat_file = temp_dir.join(format!("draftgo_launcher_{}_{}.bat", label, pid));
        let content = format!(
            "@echo off\r\necho [draftgo-launcher] Starting: {label}\r\necho.\r\n{cmd}\r\necho.\r\necho [draftgo-launcher] Command exited. Press any key to close.\r\npause >nul\r\ndel \"%~f0\" >nul 2>&1\r\n",
            label = label,
            cmd = command_line,
        );
        std::fs::write(&bat_file, &content).map_err(|e| format!("写入批处理文件失败: {e}"))?;
...
mod tests {
    ...
    mod anchored_upgrade {
        ...
        fn hermes_uses_cli_update_anchor() {
            // Hermes 自带 `hermes update`;锚定到命令行默认那处 CLI,避免 draftgo-launcher 猜
    ...
    mod install_strategy {
        ...
        fn hermes_install_uses_official_installer() {
            // Hermes 官方 installer 会处理 Python 3.11+/uv 等运行时;不要再从 draftgo-launcher
    ...
    fn resolve_launch_cwd_rejects_missing_directory() {
        let unique = std::time::SystemTime::now()
            ...
            .as_nanos();
        let missing = std::env::temp_dir().join(format!("draftgo-launcher-missing-{unique}"));
    ...
    fn iterm2_applescript_cold_start_avoids_current_window_before_one_exists() {
        let script = build_macos_iterm2_applescript(Path::new("/tmp/draftgo_launcher_launcher.sh"));
    ...
    fn ghostty_applescript_cold_start_uses_initial_command() {
        ...
        assert!(
            script.contains(r#"set launcher_command to "sh '/tmp/draftgo_launcher_launcher.sh'""#),
    ...
    fn dash_c_command_wraps_script_path_inside_quoted_arg() {
        ...
        let s = build_macos_dash_c_command(Path::new("/tmp/draftgo_launcher_launcher_1.sh"));
        assert_eq!(s, "exec sh '/tmp/draftgo_launcher_launcher_1.sh'");
