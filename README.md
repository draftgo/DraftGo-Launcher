...
### 📦 Source & Releases: **[github.com/draftgo/DraftGo-Launcher](https://github.com/draftgo/DraftGo-Launcher)**

English | [中文](README_ZH.md) | [日本語](README_JA.md) | [Deutsch](README_DE.md) | [Changelog](CHANGELOG.md)

</div>

## Why DraftGo Launcher?
...
- **Deep Link** (`draftgo://`) — Import providers, MCP servers, prompts, and skills via URL
- Dark / Light / System theme, auto-launch, auto-updater, atomic writes, auto-backups, i18n (zh/zh-TW/en/ja)

## FAQ

<details>
<summary><strong>Which AI tools does DraftGo Launcher support?</strong></summary>

DraftGo Launcher supports eight tools: **Claude Code**, **Claude Desktop**, **Codex**, **Gemini CLI**, **Grok Build**, **OpenCode**, **OpenClaw**, and **Hermes**. Each tool has dedicated provider presets and configuration management.
...
DraftGo Launcher for macOS is code-signed and notarized by Apple. You can download and install it directly — no extra steps needed. We recommend using the `.dmg` installer.
...
- **Database**: `~/.draftgo-launcher/draftgo-launcher.db` (SQLite — providers, MCP, prompts, skills)
- **Local settings**: `~/.draftgo-launcher/settings.json` (device-level UI preferences)
- **Backups**: `~/.draftgo-launcher/backups/` (auto-rotated, keeps 10 most recent)
- **Skills**: `~/.draftgo-launcher/skills/` (symlinked to corresponding apps by default)
- **Skill Backups**: `~/.draftgo-launcher/skill-backups/` (created automatically before uninstall, keeps 20 most recent)
...
If you launch from a desktop icon, add it to the `.desktop` `Exec=` line (e.g. `env DRAFTGO_LAUNCHER_GDK_BACKEND=wayland /path/to/AppImage`) or set it in your session environment. The variable is generic: on tiling Wayland compositors (sway/Hyprland) where clicks don't register, try `DRAFTGO_LAUNCHER_GDK_BACKEND=x11` instead. Leaving it unset keeps the default behavior.
...
Download the latest `DraftGo-Launcher-v{version}-Windows.msi` installer or `DraftGo-Launcher-v{version}-Windows-Portable.zip` portable version from the [Releases](../../releases) page.
...
Download `DraftGo-Launcher-v{version}-macOS.dmg` (recommended) or `.zip` from the [Releases](../../releases) page.

> **Note**: DraftGo Launcher for macOS is code-signed and notarized by Apple. You can install and open it directly.
...
- `DraftGo-Launcher-v{version}-Linux.AppImage` (Universal)
...
- **SSOT** (Single Source of Truth): All data stored in `~/.draftgo-launcher/draftgo-launcher.db` (SQLite)
